...
function ModalUpdateCategory({ categoriesData }) {
  ...
  const handleUploadMapCategory = async (e) => {
    e.preventDefault();
    const file = e.target.file.files[0];


    if (!file) return;
    const fileName = `${categoryName}-Map`;
    const storageRef = ref(storage, `Categories/Map/${fileName}`);
  ...
  return (
    <>
      {categoriesData && (
        <button style={{marginTop:"50px", padding: '5px', backgroundColor: '#0ebf22', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', marginLeft: '50px' }} onClick={handleShow}>Atualizar Categoria</button>
      )}
      {show && (
          ...
          <div style={{ zIndex: 3, backgroundColor: 'black', borderRadius: '5px', maxWidth: '500px', width: '90%', boxShadow: '0px 5px 15px rgba(0, 0, 0, 0.1)' }}>
            ...
            <div>
              <div style={{ marginBottom: '20px', height:'300px' }}>
                <label style={{ fontWeight: 'bold', color: '#007bff', display: 'block', marginBottom: '10px', marginLeft:"10px" }}>Mapa</label>
                <div style={{ width:'100%', height:'200px'}}>
                <form onSubmit={handleUploadMapCategory} style={{ display: 'flex', alignItems: 'center' }}>
                  <input type="file" name="file" style={{ marginRight: '10px', padding: '10px', fontSize: '1rem', borderRadius: '5px', border: '1px solid #ddd', boxShadow: '0px 0px 5px rgba(0, 0, 0, 0.1)' }} />
                  <button style={{ padding: '10px 20px', backgroundColor: '#034C8C', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer', boxShadow: '0px 5px 10px rgba(0, 0, 0, 0.1)' }} type="submit">Enviar</button>
                </form>
                <br />
                {!mapImage && <progress value={progress} max="100" style={{ width: '100%', height: '10px', borderRadius: '5px' }} />}
                {mapImage && <img src={mapImage} alt="Imagem do curso" style={{margin:'auto', width: '80%', height: '80%', borderRadius: '5px', boxShadow: '0px 0px 5px rgba(0, 0, 0, 0.1)' }} />}
